...
# Author: Amir HAZEM
# DATE: 01/12/2022
# Comment: This code takes as input a sequence of integers and return the same sequence in a reversed order
# Using LSTM or BiLSTM

from keras.layers import RepeatVector
from keras.layers import TimeDistributed
from keras.models import Sequential
from keras.layers import LSTM, RepeatVector, TimeDistributed, Dense, Bidirectional, SimpleRNN
import numpy as np
import sys
import random
import argparse
...
#		train_size = 20
#		seq_length = 4
#		corresponds to 20 training sequences of length 4
...
def gen_train_data(train_size, seq_length, max_int):
	...
	Y = np.array(y).reshape(train_size, seq_length, 1).astype(np.float64)
...
def train_bilstm(X, Y, hidden_size, seq_length, numb_features, epochs, batch_size):
	...
	model.add(Bidirectional(LSTM(hidden_size, activation='relu', return_sequences=True)))
	model.add(TimeDistributed(Dense(1)))
	model.compile(optimizer='adam', loss='mse')
	history = model.fit(X, Y, epochs=epochs, validation_split=0.2, verbose=1, batch_size=batch_size)
...
def param():
	...
	parser.add_argument("--seq_length", help="", type=int, default=4)
...
if __name__=='__main__':
	...
	print("Input ")
	print(list(test_input.flatten()))
	test_output = m.predict(test_input, verbose=0)
	myList = list((np.around(test_output,2)).flatten())
